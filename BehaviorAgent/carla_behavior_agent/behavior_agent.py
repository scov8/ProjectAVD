...
class BehaviorAgent(BasicAgent):
    ...
    def _other_lane_occupied(self, ego_loc, distance, check_behind=False):
        ...
        if check_behind is False:
            ...
            vehicle_state_behind, vehicle_behind, distance_behind = self._vehicle_detected_other_lane(vehicle_list, max(self._behavior.min_proximity_threshold, self._speed_limit / 3), low_angle_th=90, up_angle_th=135, check_second_lane = True)
    ...
    def run_step(self, debug=True):  # il debug era false
        ...
        if self._ending_overtake:
            ...
            if not self._local_planner.has_incoming_waypoint():
                if not self._other_lane_occupied(ego_vehicle_loc, 8, check_behind=True): #era 15 ora 7
                    print("RIENTRO")
