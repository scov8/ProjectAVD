...
class BehaviorAgent(BasicAgent):
    ...
    def run_step(self, debug=True):  # il debug era false
        ...
        vehicle_state_invasion, vehicle_invasion = self._other_lane_occupied_bis(ego_vehicle_loc, distance=30) #70mt
        if vehicle_state_invasion:
            ...
            if invasion_state:
                ...
                target_speed = min([self._behavior.max_speed, self._speed_limit]) - self._behavior.speed_decrease
                self._local_planner.set_speed(target_speed)
                control = self._local_planner.run_step(debug=debug)
                return control
        else:
