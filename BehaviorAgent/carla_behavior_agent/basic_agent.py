    ...
    def _object_obstacle_detected(self, object_list=None, max_distance=None, up_angle_th=90, low_angle_th=0, lane_offset=0):
        ...
        for target_vehicle in object_list:
            ...
            if not ego_wpt.is_junction or not target_wpt.is_junction:
                ...
                if is_within_distance(target_rear_transform, ego_front_transform, max_distance, [low_angle_th, up_angle_th]):
                    self._near_object_list.append((True, target_vehicle, compute_distance(target_transform.location, ego_transform.location)))
        ...
        if len(self._near_object_list) > 0:
            self._near_object_list = sorted(self._near_object_list,key=lambda t: t[2])
            return self._near_object_list[0]
