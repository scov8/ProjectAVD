...
class BehaviorAgent(BasicAgent):
    ...
    def run_step(self, debug=True):  # il debug era false
        ...
        vehicle_state_invasion, vehicle_invasion = self._other_lane_occupied_bis(ego_vehicle_loc, distance=30)
        if vehicle_state_invasion and not self._overtaking and not self._overtaking_obj:
        ...
        if self._ending_overtake:
            ...
            if not self._local_planner.has_incoming_waypoint():
                ...
                print(f"route_trace_p: {route_trace_p}")
                print("to find", (self._global_planner._find_closest_in_list(ego_vehicle_wp, route_trace_p) ,self._direction))
                route_trace = route_trace[(self._global_planner._find_closest_in_list(ego_vehicle_wp, route_trace_p) ,self._direction):]
            ...
            if not self._local_planner.has_incoming_waypoint():
                if not self._other_lane_occupied(ego_vehicle_loc, 15, check_behind=True): #era 15 ora 7
                    print("RIENTRO")
